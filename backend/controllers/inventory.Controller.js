...
export const getMyStockDetails = async (req, res) => {
    try {
        ...
        if (req.user.role === 'admin') {
            ...
            if (!baseId) {
                console.error('Base ID not found for user:', req.user);
                return res.status(400).json({ 
                    success: false,
                    message: 'User is not assigned to any base' 
                });
            }
            queryBase = baseId;
        }

        console.log('Querying inventory for base:', queryBase);
        ...
        if (asset) {
            ...
            console.log(`Found ${matchingAssetIds.length} matching assets for search term: ${asset}`);
        }
